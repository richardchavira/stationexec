...
from stationexec.station.events import emit_event_non_blocking, register_for_event, ActionEvents, \
    InfoEvents, emit_event
...
from stationexec.utilities.shutdown import install_signal_handlers, stop_loop, \
    MAX_WAIT_SECONDS_TILL_SHUTDOWN
from stationexec.web.handlers import ExecutiveUI, ExecutiveStaticFileHandler, \
    ShutdownHandler, ExecutiveTemplateLoader, IsAliveHandler
...
class Main(object):
    def __init__(self):
        ...
        parser.add_argument("-d", "--debug", help="set log debug level", nargs="?", const=1,
                            default=0, type=int)
        parser.add_argument("-i", "--instance", help="unique station instance designation",
                            default=None)
        parser.add_argument("-n", "--name", help="proper name of station", default=None)
        parser.add_argument("-p", "--port", help="change default HTTP server port", default=8888,
                            type=int)
        parser.add_argument("-t", "--threads", help="change default max task parallelism",
                            default=10, type=int)
        parser.add_argument("--dev", help="launch station in development mode", action="store_true")
        parser.add_argument("--alt", help="alternate home directory", type=str, required=False, default=None)
        # Optional arguments can be provided by file if desired
        parser.add_argument("-f", "--file", help="config file name or path", default=None)
        ...
        self.tornado_endpoints = [
            ...
            (r"/ui/([A-Za-z0-9\_]+)", ExecutiveUI, {"get_web_info": self._executive.get_web_info}),
            (r"/ui/([A-Za-z0-9\_]+)/([A-Za-z0-9\_]+)", ExecutiveUI,
    ...
    def start(self):
        ...
        application = tornado.web.Application(self.tornado_endpoints, **self.tornado_settings)
        # Add the custom router to handle tool-related routes
        application.add_handlers(".*", [Rule(PathMatches("/tool.*"), ToolRouter(application))])
        (web_port, ssl_config) = self._executive.get_web_info()
        self.web_server = tornado.httpserver.HTTPServer(application, ssl_options=ssl_config)

        if web_port <= 1024 and os.name != "nt" and os.geteuid() != 0:  # nt means Windows and euid == sudo
            log.error(
        ...
        try:
            ...
            log.exception("Unable to bind to port: '{0}'. Port probably in use.".
                          format(web_port), e)
            emit_event(ActionEvents.SHUTDOWN)
            sys.exit(1)

        log.info("Station Executive v{0}".format(stationexec.__version__))
        log.info("StationExec server v{1} started on {3} port {0}, pid {2}".format(
            web_port, tornado.version, os.getpid(), "http" if ssl_config is None else "https"))
        emit_event_non_blocking(InfoEvents.SERVER_STARTED, {"port": web_port,
                                                            "ssl": ssl_config is not None,
                                                            "pid": os.getpid(),
                                                            "server_version": tornado.version})

        IoLoop().current().start()
        log.info("Exited StationExec server v{1} on {3} port {0}, pid {2}".format(
            web_port, tornado.version, os.getpid(), "http" if ssl_config is None else "https"))
